...
''')
    ...
    def getAnnotationsAfterTagging(self, senna_tags, dep_parse=False):
        ...
        annotations['srl'] = zip(words, roles)
        ...
        for j in range(no_verbs):
            ...
            for curr_label in curr_labels:
                ...
                if splits[0] == 'S':
                    if len(splits) == 2:
                        if splits[1] == 'V':
                            role[splits[1]] = {'text':words[i],'span':str(i)}
                        else:
                            if splits[1] in role:
                                temp_role = role[splits[1]] + ' ' + words[i]
                                role[splits[1]] = {'text': temp_role,'span':str(i)}
                            else:
                                role[splits[1]] = {'text':words[i],'span':str(i)}
                    elif len(splits) == 3:
                        if splits[1] + '-' + splits[2] in role:
                            temp_role = role[splits[1] + '-' + splits[2]] + ' ' \
                                                                + words[i]
                            role[splits[1] + '-' + splits[2]] = {'text':temp_role,'span':str(i)}                            
                        else:
                            role[splits[1] + '-' + splits[2]] = {'text':words[i],'span':str(i)}
                elif splits[0] == 'B':
                    srl_span = '[' + str(i) + ", "
                    temp = temp + ' ' + words[i]
                elif splits[0] == 'I':
                    ...
                    if len(splits) == 2:
                        if splits[1] == 'V':
                            role[splits[1]] = {'text': temp.strip() ,'span': srl_span + str(i+1) + ']'}
                        else:
                            if splits[1] in role:
                                temp_role = role[splits[1]] + ' ' + temp
                                temp_role = \
                                    temp_role.strip()
                                role[splits[1]] = {'text':temp_role, 'span': srl_span + str(i+1) + ']'}
                            else:
                                role[splits[1]] = {'text':temp.strip(), 'span' : srl_span + str(i+1) + ']'}
                    elif len(splits) == 3:
                        if splits[1] + '-' + splits[2] in role:                            
                            ...
                            temp_role = \
                                temp_role.strip()
                            role[splits[1] + '-' + splits[2]] = {'text':temp_role,'span': srl_span + str(i+1) + ']'}
                        else:
                            role[splits[1] + '-' + splits[2]] = \
                                temp.strip()
                            temp_role = role[splits[1] + '-' + splits[2]]
